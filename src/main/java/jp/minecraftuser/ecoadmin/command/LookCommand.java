...
public class LookCommand extends CommandFrame {
    ...
    public boolean worker(CommandSender sender, String[] args) {
        ...
        if (target != null) {
            ...
            String targetNearGateLName = null;
            String targetLinkGateName = null;
            Location targetNearGateLocation = null;
            Location targetLinkGateLocation = null;
            try {
                ...
                if (targetNearGate != null) {
                    targetNearGateLName = targetNearGate.name;
                    targetNearGateLocation = targetNearGate.loc.clone();
                    targetLinkGateName = targetNearGate.name;
                    targetLinkGateLocation = targetNearGate.link.loc.clone();
                }
            } catch (Exception e) {
                return false;
            }
            ...
            //接続先ゲートが自分のワールドと同じ場合
            if (targetLinkGateLocation != null && playerLoc.getWorld() == targetLinkGateLocation.getWorld()) {
                link_gate_distance = playerLoc.distance(targetLinkGateLocation);
                gate_distance = link_gate_distance + targetLoc.distance(targetNearGateLocation);
            }
            ...
            if (gate_distance != null && player_distance != null && gate_distance < player_distance && link_gate_distance < 100) {
                ...
                targetLinkGateLocation.add(0.5, 0, 0.5);
                Vector vector = targetLinkGateLocation.toVector().subtract(playerLoc.toVector());
                ...
                sendPluginMessage(plg, sender, "§c§l§n最も近い接続先ゲート[{7}]の方向を向きました。§r\nユーザー[{0}]\n world[{1}] X[{2}] Y[{3}] Z[{4}] 距離[{5}m] \n最寄りゲート[{6}] 接続先ゲート[{7}]\n接続先ゲートworld[{8}] X[{9}] Y[{10}] Z[{11}] 距離[{12}m] \n到達予想時間[{13}s] ",
                        ...
                        targetNearGateLName,
                        targetLinkGateName,
                        targetLinkGateLocation.getWorld().getName(),
                        Integer.toString(targetLinkGateLocation.getBlockX()),
                        Integer.toString(targetLinkGateLocation.getBlockY()),
                        Integer.toString(targetLinkGateLocation.getBlockZ()),
